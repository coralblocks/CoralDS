...
public class CharMap<E> implements Iterable<E> {
    ...
    /**
     * Constructs an empty <code>CharMap</code>.
     */
	public CharMap() {
	...
	/**
	 * When using the Iterator for this <code>CharMap</code>, this method will return the current key of the last 
	...
	/**
	 * Compares this map with another CharMap by key and value.
	 *
	...
	public int hashCode() {
		int hash = 0;
		for (int i = 0; i < data.length; i++) {
			if (data[i] != null) hash += Character.hashCode((char) i) ^ data[i].hashCode();
		}
		return hash;
	}
	...
	public String toString() {
		...
		for (int i = 0; i < data.length; i++) {
			...
			sb.append(data[i] == this ? "(this CharMap)" : data[i]);
		}
		sb.append('}');
		return sb.toString();
	}
