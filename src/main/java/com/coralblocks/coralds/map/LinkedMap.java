...
/**
 * A hash-based map implementation that stores key-value pairs in insertion order.
 * <p>
 ...
 */
public class LinkedMap<K, E> implements Iterable<E> {
	...
	private Entry<K, E> head;

	private Entry<K, E> tail;
	...
	/**
	 * Creates a <code>LinkedMap</code> with the default initial capacity and load factor.
	 */
	public LinkedMap() {
	...
	/**
	 * Creates a <code>LinkedMap</code> with the default load factor.
	 *
	...
	/**
	 * Creates a <code>LinkedMap</code> with the specified initial capacity and load factor.
	 *
	 * @param initialCapacity the desired initial capacity
	...
	/**
	 * When using the Iterator for this <code>LinkedMap</code>, this method will return the current key 
	...
	private void addToLinkedList(Entry<K, E> entry) {
		if (tail == null) {
			head = entry;
			tail = entry;
		} else {
	...
	private void removeFromLinkedList(Entry<K, E> entry) {
	 ...
	 * If the key is already present, the old value is replaced without changing
	 * the key's iteration position.
	 *
	...
	/**
	 * Returns an iterator that reuses the same instance (garbage-free). Entries are
